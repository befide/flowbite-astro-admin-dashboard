...
export const allCommunityTopLevelOrganizations = async () =>
  await getCollection("organizations", (entry) => entry.data.partOfCommunityDegree !== "none" && entry.data.parent__id && !entry.data.topLevel__id && entry.data.befideOrganizationCategories.indexOf("committee") !== 0)

export const allCommunityOrganizations = async () => await getCollection("organizations", (entry) => entry.data.partOfCommunityDegree !== "none" && entry.data.befideOrganizationCategories.indexOf("committee") !== 0)

export const getOrganizationCategories = async () => Array.from(new Set((await allCommunityTopLevelOrganizations()).flatMap((entry) => entry.data.befideOrganizationCategories)))
