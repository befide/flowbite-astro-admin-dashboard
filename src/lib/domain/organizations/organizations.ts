...
export const allOrganizations = async () =>
  (await getCollection("organizations")).map(
    ({ data }) => data,
  )

export const allOrganizationsForTopLevelOrganization =
  async (topLevel__id: string) => {
...
export const allCommunityTopLevelOrganizations = async () =>
  await getCollection(
    ...
    (entry) =>
      ...
      entry.data.befideOrganizationCategories.indexOf(
        "committee",
      ) !== 0,
  )
...
export const organizationsForAPI = async (
  locale: string,
) => {
  const organizations =
  ...
  return await Promise.all(
    organizations
      ...
      .map(
        async (organization) =>
          await new Organization(organization.data).getDto(
            locale,
          ),
      ),
  )
