...
export const allCommunityOrganizations = async () =>
  await getCollection(
    "organizations",
    (entry: CollectionEntry<"organizations">) =>
      entry.data.partOfCommunityDegree !== "none" && entry.data.category !== "committee",
  )
...
export const allCommunityGroups = async () =>
  await getCollection(
    ...
    (entry: CollectionEntry<"organizations">) =>
      entry.data.partOfCommunityDegree !== "none" &&
      entry.data.instanceOfs__taxonomyID.indexOf("/g/organization/working-group") > -1,
  )
...
export const organizationsForAPI = async (locale: "en" | "de") => {
  ...
  return await Promise.all(
    organizations
      // .filter((o) => o.id !== ":")
      .map(async (organization) => await new Organization(organization.data).getDto(locale)),
  )
}
