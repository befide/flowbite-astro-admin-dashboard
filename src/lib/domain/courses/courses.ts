...
export async function allCoursesData(): Promise<CourseSchema[]> {
  return (await getCollection("courses")).map((d: CollectionEntry<"courses">) => d.data).sort((a:CourseSchema, b:CourseSchema) => ascending(a.id, b.id))
}
...
export async function coursesForAPI(locale = "en"): Promise<CourseDto[]> {
  ...
  return await Promise.all(
    courses.map(async (course) => await new Course(course).getDto(locale))
  )
}
