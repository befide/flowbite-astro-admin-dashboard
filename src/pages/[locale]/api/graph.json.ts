...
const peopleKeys = fastCartesian([
  [...careerLevels],
  [...disciplinaryProfessions],
  [...genders],
]).map((p) => p.join("."));
...
export const GET: APIRoute = async ({ props }) => {
  ...
  const community = await Promise.all(
    organizations
      .filter((o) => o.id !== ":")
      .map(async (organization) => await new Organization(organization.data)),
  );
  ...
  const organizationNodes = community
    ...
    .map((o: Organization) => {
      const subtype =
        o._data.instanceOfs__taxonomyID.indexOf(
  ...
  organizationNodes.sort(
    (a, b) => (a.isUniversity ? -1 : 1) * a.label.localeCompare(b.label),
  );
  organizationNodes
  ...
  const peopleNodes = community
    .flatMap((o: Organization) => {
      return peopleKeys
        ...
        .flatMap((p) =>
          new Array(getValue(o._data.uniquePeopleCount, p))
            .fill(p)
            .map((d) => ({
              group: o._data.id,
              affiliation: o._data.topLevel__id,
              type: "person",
              subtype: p,
            })),
        );
  ...
  const groupLinks = peopleNodes.map((p) => ({
    source: p.id,
    target: p.group,
    subtype: p.subtype,
    type: "group-member",
  }));
  const affiliationLinks = peopleNodes.map((p) => ({
    source: p.id,
    target: "community/" + p.affiliation,
    type: "has-affiliation",
  }));
