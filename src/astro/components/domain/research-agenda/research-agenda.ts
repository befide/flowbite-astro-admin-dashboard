...
export type ResearchAgendaItemHierarchyNode =
  HierarchyNode<ResearchAgendaItem>

export const createResearchAgenda = (
  filter = "agenda.facilities",
) => {
  const dTheta = 0.0
  ...
  let researchAgenda: ResearchAgendaItem[] = Object.values(
    _data,
  ).map((d) => ({
    ...d,
  ...
  const itemsById = new Map<string, any>(
    researchAgenda.map((d) => [d.id, d]),
  )

  researchAgenda.forEach((d) => {
    d.links = d.links.map(
      (id, i) => itemsById.get(id)?.path,
    )
  })

  const stratified: HierarchyNode<ResearchAgendaItem> =
    stratify<ResearchAgendaItem>().path(
      (d: ResearchAgendaItem) =>
        d.path.replaceAll(".", "/"),
    )(researchAgenda)
  ...
  clusterLayout.leaves().forEach((d) => {
    if (d.data.path.includes(filter))
      activeElements.add(d.data.path)
  })
  ...
  const gradientElements = defsElement
    ...
    .attr("x1", ({ source, target }) => {
      return (
        radius *
        Math.cos(
          clusterLayoutLeavesByPath.get(source)?.x -
            Math.PI / 2,
        )
      )
    })
    .attr(
      ...
      ({ source }) =>
        radius *
        Math.sin(
          clusterLayoutLeavesByPath.get(source)?.y -
            Math.PI / 2,
        ),
    )
    .attr(
      ...
      ({ target }) =>
        radius *
        Math.cos(
          clusterLayoutLeavesByPath.get(target)?.x -
            Math.PI / 2,
        ),
    )
    .attr(
      ...
      ({ target }) =>
        radius *
        Math.sin(
          clusterLayoutLeavesByPath.get(target)?.y -
            Math.PI / 2,
        ),
    )
  ...
  const nodesElement = svg
    .append("g")
    .classed("nodes", true)
  const linksElement = svg
    .append("g")
    .classed("links", true)
  ...
  const nodeElementsText = nodeElements
    ...
    .attr("transform", ({ x }) =>
      x >= Math.PI ? "rotate(180)" : null,
    )
    .attr("fill", color)

  nodeElementsText
    .append("tspan")
    .text((d: HierarchyPointNode<ResearchAgendaItem>) =>
      d.x <= Math.PI ? d.data.group || "" : d.data.label,
    )
  ...
  nodeElementsText
    ...
    .text((d) =>
      d.x >= Math.PI
        ? nameFixer(d.data.group)
        : nameFixer(d.data.label),
    )
  ...
  const linkElements = linksElement
    ...
    .attr("stroke-width", ({ source, target }) =>
      filter === "" ||
      (activeElements.has(source) &&
        activeElements.has(target))
        ? 3
        : 3,
    )
    ...
    .attr("d", (d) => {
      return ribbon()({
        source: {
          startAngle:
            clusterLayoutLeavesByPath.get(d.source)?.x -
            dTheta,
          endAngle:
            clusterLayoutLeavesByPath.get(d.source)?.x +
            dTheta,
          radius: clusterLayoutLeavesByPath.get(d.source)
            ?.y,
        },
        target: {
          startAngle:
            clusterLayoutLeavesByPath.get(d.target)?.x -
            dTheta,
          endAngle:
            clusterLayoutLeavesByPath.get(d.target)?.x +
            dTheta,
  ...
  const objectiveNodes = clusterLayout
    .leaves()
    .filter((d) =>
      d.data.path?.startsWith("agenda.objectives"),
    )

  const facilityNodes = clusterLayout
    .leaves()
    .filter((d) =>
      d.data.path?.startsWith("agenda.facilities"),
    )
  ...
  svg
    ...
    .datum(({ startAngle, endAngle, level, height }) => ({
      ...
      outerRadius:
        radius + (level - 1 + height) * baselineHeight,
    }))
    .attr("stroke", "white")
    .attr("d", arc())
...
function updateFilter(nodeElements: any) {
  ...
  nodeElements
    .attr(
      "fill-opacity",
      (d: ResearchAgendaItemHierarchyNode) =>
        filter === "" || activeElements.has(d.data.path)
          ? 1
          : 0.1,
    )
