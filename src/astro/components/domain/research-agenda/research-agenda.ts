...
export const createResearchAgenda = (filter = "agenda.facilities") => {
  ...
  function color(d: HierarchyNode<ResearchAgendaItem>) {
    return scaleOrdinal(["topics", "facilities", "objectives"], [red.toString(), blue.toString(), green.toString()])(d.data.ancestors![2])
  }
  ...
  const stratified: HierarchyNode<ResearchAgendaItem> = stratify<ResearchAgendaItem>().path((d: ResearchAgendaItem) => d.path.replaceAll(".", "/"))(researchAgenda)

  const clusterLayout = cluster<ResearchAgendaItem>()
    .size([2 * Math.PI, radius])
    .separation((a, b) => {
      if (a.data.ancestors && b.data.ancestors && a.data.ancestors[2] !== b.data.ancestors[2]) return 3
      if (a.data.ancestors && b.data.ancestors && a.data.ancestors[1] !== b.data.ancestors[1]) return 2
      return 1
    })(stratified)
  ...
  const filterElement = defsElement.append("filter").attr("id", "shadow").attr("color-interpolation-filters", "sRGB")
  filterElement.append("feDropShadow").attr("dx", 0).attr("dy", 0).attr("stdDeviation", 3).attr("flood-opacity", 0.5)
  ...
  const gradientElements = defsElement
    ...
    .attr("x1", ({ source, target }) => {
      return radius * Math.cos(clusterLayoutLeavesByPath.get(source)?.x - Math.PI / 2)
    })
    .attr("y1", ({ source }) => radius * Math.sin(clusterLayoutLeavesByPath.get(source)?.y - Math.PI / 2))
    .attr("x2", ({ target }) => radius * Math.cos(clusterLayoutLeavesByPath.get(target)?.x - Math.PI / 2))
    .attr("y2", ({ target }) => radius * Math.sin(clusterLayoutLeavesByPath.get(target)?.y - Math.PI / 2))

  gradientElements
    .append("stop")
    .attr("offset", "0%")
    .attr("stop-color", ({ source }) => clusterLayoutLeavesByPath.get(source) && color(clusterLayoutLeavesByPath.get(source)))
  gradientElements
    .append("stop")
    .attr("offset", "100%")
    .attr("stop-color", ({ target }) => clusterLayoutLeavesByPath.get(target) && color(clusterLayoutLeavesByPath.get(target)))
  ...
  const nodeElements = nodesElement
    ...
    .attr("fill-opacity", (d) => (filter === "" || activeElements.has(d.data.path) ? 1 : 0.1))
    .attr("fill", (d) => (filter === "" || activeElements.has(d.data.path) ? color(d) : "#f0f0f0"))
  ...
  nodeElementsText
    ...
    .on("mouseover", (_, d) => {
      filter = (d.x <= Math.PI ? d.data.ancestors?.slice(1).reverse().join(".") : d.data.ancestors?.slice(0).reverse().join(".")) || ""
    })
  nodeElementsText
    .append("tspan")
  ...
  const linkElements = linksElement
    ...
    .classed("link", true)
    .attr("stroke-width", ({ source, target }) => (filter === "" || (activeElements.has(source) && activeElements.has(target)) ? 3 : 3))
    .attr("stroke-opacity", ({ source, target }) => (filter === "" || (activeElements.has(source) && activeElements.has(target)) ? 0.3 : 0.0))
    .attr("d", (d) => {
      return ribbon()({
        source: {
...
function updateFilter(nodeElements: any) {
  ...
  // });
  nodeElements
    .attr("fill-opacity", (d: ResearchAgendaItemHierarchyNode) => (filter === "" || activeElements.has(d.data.path) ? 1 : 0.1))
    .attr("fill", (d: ResearchAgendaItemHierarchyNode) => (filter === "" || activeElements.has(d.data.path) ? color(d) : "#f0f0f0"))
  ...
  linkElements
    .attr("stroke-width", ({ path = "" }) => (filter === "" || activeElements.has(path) ? 3 : 3))
    .attr("stroke-opacity", ({ id = "" }) => (filter === "" || activeElements.has(id) ? 0.3 : 0.0))
    .style("stroke", ({ id = "" }) => (filter === "" || activeElements.has(id) ? `url(#${gradientId({ id })})` : "#999999"))
}
